...
public sealed class VolumeSetCompletenessTests
{
    ...
    [Fact]
    public void ThreeDigitAndSVolumeGaps_AreCounted()
    {
        var present = Names("rar", "r000", "r002", "s00", "s02", "s03");

        // r001 and s01.
        Assert.Equal(2, VolumeSetReadiness.CountMissingVolumes(Base, present, null));
    }
    ...
    public void ReadSfvDeclaredNames_SkipsCommentsAndBlankLines()
    {
        ...
        try
        {
            ...
            Assert.Equal(new[] { $"{Base}.rar", $"{Base}.r00" }, names);
        }
    ...
    [Fact]
    public void LoneFirstVolumeShortcut_ConsultsTheSfv()
    {
        var code = ExtractorCode();
        var start = code.IndexOf("Task<ArchiveWaitOutcome> WaitForVolumeSetReady(", StringComparison.Ordinal);
        var body = code[start..code.IndexOf("var previous = SampleVolumeSet(path);", start, StringComparison.Ordinal)];

        Assert.Matches(@"is not \{ Count: > 1 \}\s*&&\s*VolumeSetReadiness\.CountMissingVolumes\(", body);
    }
