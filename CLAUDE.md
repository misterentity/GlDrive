...
GlDrive — a Windows 11 tray app that mounts glftpd FTPS servers as local drive letters using WinFsp + FluentFTP + GnuTLS. Single project (`src/GlDrive/GlDrive.csproj`), .NET 10 WPF targeting win-x64, plus a unit-test project (`src/GlDrive.Tests/GlDrive.Tests.csproj`, 1250 tests via `dotnet test`) covering the pure logic (crypto round-trips, login-gate accounting, spread scoring, zipscript artifact filtering, etc.). The WinFsp/WPF/live-FTP layers still require manual runtime verification — `dotnet build` + exercise the app.
...
- **AiAgent** (`AiAgent/`, ~46 files) — LLM-driven self-tuning loop. `TelemetryRecorder` appends per-stream JSONL events (`%AppData%\GlDrive\ai-data\{stream}-{date}.jsonl`) for races, nukes, announces (matched + no-match), section activity, downloads, etc. `AgentRunner` (wired in `App.xaml.cs`, daily at `AgentConfig.RunHourLocal`, default 04:00, plus manual `RunNowAsync`) calls `LogDigester.Build` → deterministic `*Digester`s → compact `DigestBundle` → `AgentClient` (Claude Sonnet via OpenRouter) → JSON `AgentChange` proposals → `ChangeApplier`. **Every mutation is gated**: frozen-path check (`FreezeStore`), confidence threshold, per-category/total budgets, then a per-category `Validators/*` (`SectionMappingValidator`, `SkiplistValidator`, …) that owns the actual `AppConfig` mutation. `DryRunsRemaining` (default 3) gates the first live applies; `AuditTrail` records before/after + rejection reason for undo. **Section→folder learning (v3.7.0)**: `MatchedAnnounceEvent` captures (release-type via `SceneNameParser` → IRC section) on every matched announce; enriched `RaceOutcomeEvent` records the resolved destination folder; `SectionFolderDigester` builds a co-occurrence table the agent uses to propose discriminating `SectionMapping` triggers. `SpreadJob` routes via `SectionMapper.Resolve` (before the fuzzy fallback) so learned mappings actually take effect. **Invariant**: never mutate `AppConfig` outside a validator; `SectionMappingValidator` only patches default (`.*`/empty) triggers — user-edited triggers are preserved — and rejects appends whose `RemoteSection` isn't a known `Sections` key.
