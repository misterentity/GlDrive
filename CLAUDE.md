...
GlDrive — a Windows 11 tray app that mounts glftpd FTPS servers as local drive letters using WinFsp + FluentFTP + GnuTLS. Single project (`src/GlDrive/GlDrive.csproj`), .NET 10 WPF targeting win-x64, plus a unit-test project (`src/GlDrive.Tests/GlDrive.Tests.csproj`, 413 tests via `dotnet test`) covering the pure logic (crypto round-trips, login-gate accounting, spread scoring, zipscript artifact filtering, etc.). The WinFsp/WPF/live-FTP layers still require manual runtime verification — `dotnet build` + exercise the app.
